...
void StageSelectScene::Initialize()
{
	...
	ModelManager::GetInstans()->LoadModel("Text_1-1.obj");
	ModelManager::GetInstans()->LoadModel("Text_1-2.obj");
	...
	Player_->SetRotate({ 0, std::numbers::pi_v<float> / 2.0f , 0 });
	Player_->SetLighting(false);

	// 作成してunique_ptrでリストに追加
	for (uint32_t i = 0; i < MaxSelectIndex_; ++i) {
		...
		if (i == 0) {
			newObject->SetModel("Text_1-1.obj");
		}else if (i == 1) {
			newObject->SetModel("Text_1-2.obj");
		} else {
			newObject->SetModel("Text_1-1.obj");
		}
		newObject->SetTranslate(Vector3(7.0f * i, 0.0f, 0.0f)); // X座標を変更して配置
		newObject->SetLighting(false);
		selectObjects_.push_back(std::move(newObject));
	}
	...
	startPos_ = selectObjects_.front()->GetTranslate();  // 移動開始位置
	endPos_ = selectObjects_.at(1)->GetTranslate();  // 移動終了位置（例えば2番目のオブジェクトへ）
	easingDuration_ = 2.0f;  // イージングの期間（秒）

	TextureManager::GetInstance()->LoadTexture("Resources/controllerUI.png");

	// コントローラ操作のUI
	uIController_ = std::make_unique <Sprite>();
	uIController_->Initialize(SpriteCommon::GetInstance(), "Resources/controllerUI.png");
...
void StageSelectScene::move() {
	...
	if ((rightStickX > stickThreshold || (continueMove && rightStickX > stickThreshold))
		...
		startPos_ = selectObjects_[currentIndex_ - 1]->GetTranslate();  // 直前の位置
		endPos_ = selectObjects_[currentIndex_]->GetTranslate();        // 新しい位置
...
void StageSelectScene::moveChangeScene() {
	...
	if (!easingsceneFlag_ && !easingmoveFlag_) {
		...
		if (Input::GetInstance()->TriggerGamePadButton(XINPUT_GAMEPAD_A)) {
			...
			// selectObjects_ の現在位置にカメラを移動
			Vector3 selectObjectPos = selectObjects_[currentIndex_]->GetTranslate();
			// mainObject の位置を selectObjects_ の位置に設定
			Player_->SetTranslate(selectObjectPos);
			// カメラのターゲットを現在選択されているオブジェクトに設定
			CameraManager::GetInstans()->GetCamera("maincam")->SetFollowTarget(selectObjects_[currentIndex_].get(), { 0, 0, -15 });
			CameraManager::GetInstans()->GetCamera("maincam")->SetFollowMode(true);
	...
	if (easingsceneFlag_) {
		...
		Camera* activeCam = CameraManager::GetInstans()->GetActiveCamera();
		if (activeCam == CameraManager::GetInstans()->GetCamera("maincam")) {
			activeCam->SetFollowTarget(selectObjects_[currentIndex_].get(), Vector3(0.0f, 0.0f, -15.0f));
